...
def tracer_occupation_parking(nom_fichier, nom_parking):
    ...
    plt.title(f"Occupation du parking voiture {nom_parking} au cours du temps")
...
tracer_occupation_parking('Donnée_SAE_Parking', 'Gaumont OUEST')

def tracer_occupation_velo(nom_fichier, nom_parking):
    try:
        with open(nom_fichier, 'r', encoding='utf-8') as f:
            donnees = json.load(f)
    except FileNotFoundError:
    ...
    plt.figure(figsize=(10, 5))
    plt.plot(temps_list, occupation_list, marker='o', color='red', markersize=4)
    plt.title(f"Occupation du parking velo {nom_parking} au cours du temps")
    plt.xlabel("Date et Heure")
    plt.ylabel("Occupation (%)")
    plt.xticks(rotation=45)
    plt.grid(True)
...
tracer_occupation_velo('Donnée_SAE_velo', 'Odysseum')
